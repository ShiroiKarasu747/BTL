...
namespace QuanLyPhongMay
{
    public partial class frmTimKiemMayTinh : Form
    {
        ...
        private void Load_comboBox()
        {
            ...
            cboMaPhongMay.DataSource = QuanlyPhongMay.Class.Functions.GetDataToTable("SELECT MaPM, TenPM FROM tblPhongMay");
            cboMaPhongMay.ValueMember = "MaPM";
            cboMaPhongMay.DisplayMember = "TenPM";
            cboMaPhongMay.SelectedIndex = -1;

            //Load cboMaChip tìm kiếm
            cboMaChip.DataSource = QuanlyPhongMay.Class.Functions.GetDataToTable("SELECT MaChip, TenChip FROM tblChip");
            cboMaChip.ValueMember = "MaChip";
            cboMaChip.DisplayMember = "TenChip";
            cboMaChip.SelectedIndex = -1;

            //Load cboMaRam tìm kiếm
            cboMaRam.DataSource = QuanlyPhongMay.Class.Functions.GetDataToTable("SELECT MaRam, TenRam FROM tblRAM");
